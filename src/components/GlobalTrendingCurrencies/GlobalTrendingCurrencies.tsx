...
export default function GlobalTrendingCurrencies(props: GlobalTrendingCurrenciesProps) {
    ...
    function formatMarketCapPercentageTopTwoCurrencies(currencies: {[key: string]: number}): string {
        return Object.keys(currencies).map((marketPercentage, index) => {
            if (index >= 2) return
            ...
            {props.currencies.data ? (
               <article className="h-16 flex justify-center items-center flex-1 text-black">
                    <div className="flex flex-row text-white">
                        <div className="mr-3">Moedas: <span className="text-blue-500">{props.currencies.data.active_cryptocurrencies}</span></div>
                        <div className="mr-3">Câmbios: <span className="text-blue-500">{props.currencies.data.markets}</span></div>
                        <div className="mr-3">Capitalização de mercado: <span className="text-blue-500">{formatter.format(getTotalMarketCap)}</span></div>
                        <div className="mr-3">Volume 24h: <span className="text-blue-500">{formatter.format(getTotalVolume)}</span></div>
                        <div className="mr-3">Domínio:  
